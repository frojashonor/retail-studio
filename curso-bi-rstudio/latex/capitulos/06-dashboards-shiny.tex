...
``enmascaran'' funciones como \codigo{box()}, \codigo{renderDataTable()} o
\codigo{filter()}; son normales):
...
\begin{tikzpicture}[
  ...
  \draw[->, thick, acento] (in1.east) to[out=10, in=170]
       node[above, yshift=8pt, font=\sffamily\scriptsize, text=acento] {1. el usuario cambia un valor} (inr.west);
...
\begin{tikzpicture}[font=\sffamily\small]
  ...
  \draw[<-, acento, thick] (4.4,2.35) -- (9.4,2.35) node[right, font=\sffamily\footnotesize] {\codigo{numericInput("precio")}};
  \draw[<-, acento, thick] (4.4,1.2) -- (9.4,1.2) node[right, font=\sffamily\footnotesize] {\codigo{sliderInput("descuento")}};
  \draw[<-, exito, thick] (3.6,0.4) -- (9.4,0.4) node[right, font=\sffamily\footnotesize] {\codigo{textOutput("precio\_final")}};
\end{tikzpicture}
...
\codigo{Listening on} \path{http://127.0.0.1:XXXX}) y no acepta otros comandos. Para
...
Aquí hay una trampa en la que cae casi todo el mundo. Cuando ejecutas \codigo{runApp()} con la carpeta
\path{latex/codigo/apps/02-explorador-ventas}, Shiny
...
\codigo{plotlyOutput()} & \codigo{renderPlotly()} & Una
...
En una app real ese error aparece al abrirla. Por ejemplo, si en el cuerpo
del \codigo{server} escribes \codigo{doble <- input\$n * 2}, la consola
muestra
...
\begin{tikzpicture}[font=\sffamily\footnotesize,
  ...
  % main
  \node[anchor=west, font=\sffamily\bfseries] at (3.7,6.3)
  ...
  \node[text=acento, anchor=east, font=\sffamily\scriptsize] at (12.8,6.8) {resumen: \codigo{textOutput("resumen")}};
\end{tikzpicture}
...
\begin{tikzpicture}[font=\sffamily\footnotesize,
  ...
  \node[anchor=west, text=white, align=left] at (0.2,3.95) {Periodo:\\[1pt]
    \colorbox{white}{\textcolor{black}{01/01/23 a 31/12/23}}};
  ...
  \node[white, anchor=west, font=\sffamily\scriptsize] at (9.9,5.22) {Método de pago};
...
gráfica de productos y la de categorías (\codigo{grafica\_categorias()},
en el archivo, junto con \codigo{grafica\_metodo\_pago()} de la pestaña
Resumen): ambas resumen a dos columnas,
...
\begin{rnoejecutar}[title={app.R (parte 4): cuerpo (extracto)}]
...
\begin{rnoejecutar}[title={app.R (partes 6 a 8): salidas, descargas y lanzamiento (extracto)}]
...
(ya cargamos \paquete{plotly} al inicio del módulo):
...
ejemplo, \path{latex/codigo/apps/02-explorador-ventas} a
\path{mis-apps/02-mi-explorador}) y trabaja sobre la copia. Además de
