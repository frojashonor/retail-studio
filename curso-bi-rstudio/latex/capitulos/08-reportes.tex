...
\begin{tikzpicture}[
  archivo/.style={draw=primario, fill=primario!6, rounded corners=3pt,
                  minimum width=1.9cm, minimum height=1cm, align=center,
                  font=\sffamily\footnotesize},
  motor/.style={draw=acento, fill=acento!10, ellipse, minimum width=1.9cm,
  ...
  \node[motor, right=0.7cm of rmd] (knitr) {knitr};
  \node[archivo, right=0.7cm of knitr] (md) {\textbf{.md}\\texto +\\resultados};
  \node[motor, right=0.7cm of md] (pandoc) {pandoc};
  \node[salida, right=1.1cm of pandoc, yshift=1.05cm] (html) {HTML};
  \node[salida, right=1.1cm of pandoc] (docx) {Word (.docx)};
  \node[salida, right=1.1cm of pandoc, yshift=-1.05cm] (tex) {LaTeX};
  \node[salida, right=0.5cm of tex] (pdf) {PDF};
...
vendido en el año fue de \textbf{\$TOTALANUAL}''. Si mañana llegan más
...
8 transacciones (ticket de \$1,664.64), mientras la Sucursal Norte necesitó
...
frase_variacion <- function(actual, anterior, periodo = "el mes anterior") {
  ...
  if (is.na(anterior) || anterior == 0) {
    return(paste0("Las ventas fueron de ", dollar(actual, accuracy = 1),
                  "; no hay datos de ", periodo, " para comparar."))
  }
  ...
  paste0("Las ventas ", verbo, " respecto a ", periodo, " (de ",
...
ventas. Pero cuidado con la interpretación: cada tienda tiene solo entre 5 y
15 transacciones al mes en estos datos, así que variaciones tan grandes
...
\begin{enumerate}
  ...
  \item Guárdalo como \archivo{plantilla\_corporativa.docx} junto al
        \archivo{.Rmd} y agrega la opción \codigo{reference\_docx}.
\end{enumerate}
...
for (i in seq_len(nrow(pendientes))) {
  resultado <- tryCatch({
    archivo <- generar_uno(pendientes$tienda[i], pendientes$mes[i])
    paste("OK:", archivo)
  }, error = function(e) {
    paste("ERROR:", conditionMessage(e))   # el mensaje del error
  })
  registro[[i]] <- tibble(tienda = pendientes$tienda[i],
                          mes = pendientes$mes[i],
...
\begin{enumerate}
  ...
  \item En \menu{Programa o script} escribe la ruta de Rscript, por ejemplo
        \archivo{C:\textbackslash Program Files\textbackslash
        R\textbackslash R-4.3.3\textbackslash bin\textbackslash Rscript.exe}.
...
vendidos, lista para ejecutarse con
...
La tienda `r params$tienda` vendió $`r round(sum(ventas_tienda$total_transaccion))`
en 2023.
...
sección~\ref{sec:m8-directorio}), lee y une las tablas, valida los
parámetros y filtra la tienda y los meses:

\begin{textocodigo}[title=Bloque datos (después de localizar la carpeta)]
```{r datos}
# ... (búsqueda de carpeta_datos y lectura de transacciones, tiendas
#      y productos, como en la sección de parámetros) ...
...
\begin{textocodigo}[title=Bloque tabla-kpis]
```{r tabla-kpis}
...
Generando 10 reportes del mes 2023-12 en formato pdf
  [ 1/10] Sucursal Centro    OK    2023-12_T01_sucursal-centro_FECHA.pdf
  [ 2/10] Sucursal Norte     OK    2023-12_T02_sucursal-norte_FECHA.pdf
  ...
\end{textocodigo}
