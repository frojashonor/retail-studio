...
En \codigo{ventas} el vendedor se identifica con \codigo{vendedor\_id}, pero
...
\codigo{join\_by()} se lee más natural y además permite uniones por
...
R distingue mayúsculas de minúsculas. Revisa los nombres exactos con
\codigo{names(ventas)} o \codigo{glimpse(ventas)}.
...
\textbf{Solución}: homologa los tipos antes de apilar, por ejemplo con
\codigo{mutate(monto = parse\_number(monto))} en la tabla de texto.
