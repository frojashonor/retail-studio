...
La tabla~\ref{tab:m0-ciclo} detalla qué haces en cada paso y qué
herramientas de R usarás. No te preocupes si todavía no reconoces los
nombres: los irás conociendo módulo a módulo.

\begin{table}[H]
\centering\small
\caption{Qué se hace en cada paso del ciclo y con qué herramientas de R.}
\label{tab:m0-ciclo}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{3.0cm}
    >{\raggedright\arraybackslash}X
    >{\raggedright\arraybackslash}p{5.2cm}@{}}
\toprule
\textbf{Paso} & \textbf{Qué haces} & \textbf{Herramientas en R} \\
\midrule
1. Pregunta &
  Acuerdas con el área de negocio qué se quiere decidir y cómo se medirá
  el éxito. &
  Papel, lápiz y buenas preguntas. \\
2. Obtener datos &
  Localizas las fuentes y las cargas en R. &
  \paquete{readr}, \paquete{readxl}, \paquete{DBI}, \paquete{RSQLite} \\
3. Limpiar y transformar &
  Corriges tipos y errores, unes tablas y calculas columnas nuevas. A este
  proceso se le llama \termino{ETL} (\textit{extract, transform, load}:
  extraer, transformar y cargar). &
  \paquete{dplyr}, \paquete{tidyr}, \paquete{lubridate},
  \paquete{dbplyr} \\
4. Analizar &
  Resumes, comparas, buscas patrones y construyes modelos. &
  \paquete{dplyr}, \paquete{skimr}, \paquete{forecast},
  \paquete{caret}, \paquete{cluster} \\
5. Visualizar y comunicar &
  Conviertes los resultados en gráficas, tableros y reportes que
  cualquiera entiende. &
  \paquete{ggplot2}, \paquete{shiny}, \paquete{plotly},
  \paquete{rmarkdown} \\
6. Decidir &
  El negocio decide con base en la evidencia; tú explicas alcances y
  riesgos. &
  Un buen resumen ejecutivo. \\
7. Medir &
  Compruebas si la decisión funcionó con los mismos KPI, periódicamente. &
...
(tabla~\ref{tab:m0-formato}).

\begin{table}[H]
\centering\small
\caption{Formato del texto y su significado.}
\label{tab:m0-formato}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{5.2cm} X@{}}
\toprule
\textbf{Así lo verás} & \textbf{Significa} \\
\midrule
\codigo{summarise()} & Código de R dentro del texto: funciones, objetos,
  argumentos. \\
\paquete{dplyr} & Nombre de un paquete de R. \\
\archivo{ventas\_retail.csv} & Nombre de un archivo o de una carpeta. \\
\menu{Tools > Global Options} & Menú u opción de RStudio. \\
\tecla{Ctrl} + \tecla{Enter} & Atajo de teclado. En Mac, cambia
  \tecla{Ctrl} por \tecla{Cmd}. \\
\termino{tabla de hechos} & Término nuevo que se define ahí mismo. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{enumerate}
  ...
  \item Instálalo: en Windows ejecuta el \archivo{.exe}; en macOS abre el
        \archivo{.dmg} y arrastra RStudio a \archivo{Aplicaciones}; en
        Ubuntu instala el archivo \archivo{.deb}.
...
\subsection{Un recorrido rápido por RStudio}

Al abrir RStudio verás cuatro paneles (figura~\ref{fig:m0-rstudio}). Si solo
ves tres, abre un script nuevo con \menu{File > New File > R Script} y
aparecerá el editor.

\begin{figure}[H]
\centering
\begin{tikzpicture}[
  titulo/.style={font=\sffamily\bfseries\small, text=primario,
    anchor=north west},
  texto/.style={font=\sffamily\scriptsize, text=gristexto,
    anchor=north west, align=left},
]
  % Ventana
  \draw[gristexto!70, fill=gray!8, rounded corners=3pt]
    (0,0) rectangle (14.4,8.3);
  \fill[primario, rounded corners=3pt] (0,7.75) rectangle (14.4,8.3);
  \fill[primario] (0,7.75) rectangle (14.4,7.95);
  \node[anchor=west, font=\sffamily\footnotesize\bfseries, text=white]
    at (0.2,8.02) {RStudio};
  \node[anchor=west, font=\sffamily\scriptsize, text=gristexto]
    at (0.2,7.45) {File\quad Edit\quad Code\quad View\quad Plots\quad
    Session\quad Build\quad Debug\quad Profile\quad Tools\quad Help};
  % Paneles
  \draw[fill=white, draw=gristexto!50] (0.2,3.85) rectangle (8.4,7.15);
  \draw[fill=white, draw=gristexto!50] (0.2,0.2) rectangle (8.4,3.65);
  \draw[fill=white, draw=gristexto!50] (8.6,3.85) rectangle (14.2,7.15);
  \draw[fill=white, draw=gristexto!50] (8.6,0.2) rectangle (14.2,3.65);
  % 1. Editor
  \node[titulo] at (0.35,7.0) {1. Editor (Source)};
  \node[texto, text width=7.6cm] at (0.35,6.45)
    {Aquí escribes y guardas tus scripts (\texttt{.R}) y reportes
     (\texttt{.Rmd}). Nada se ejecuta hasta que lo envías a la consola con
     \texttt{Ctrl + Enter}.};
  \node[anchor=north west, font=\ttfamily\scriptsize, text=rcomentario,
    align=left] at (0.35,5.2)
    {\# Mi primer script\\
     \textcolor{rtexto}{ventas <- c(1200, 950, 1830)}\\
     \textcolor{rtexto}{sum(ventas)}};
  % 2. Consola
  \node[titulo] at (0.35,3.5) {2. Consola (Console)};
  \node[texto, text width=7.6cm] at (0.35,2.95)
    {Aquí R ejecuta el código y responde. El símbolo \texttt{>} indica
     que está listo para recibir una instrucción.};
  \node[anchor=north west, font=\ttfamily\scriptsize, text=rtexto,
    align=left] at (0.35,1.95)
    {> sum(ventas)\\
     \textcolor{black!80}{[1] 3980}\\
     >};
  % 3. Entorno
  \node[titulo] at (8.75,7.0) {3. Entorno (Environment)};
  \node[texto, text width=5.2cm] at (8.75,6.45)
    {Los objetos que has creado: tablas, vectores, modelos. Las pestañas
     \textit{History} y \textit{Connections} guardan tu historial y tus
     conexiones a bases de datos.};
  \node[anchor=north west, font=\ttfamily\scriptsize, text=rtexto]
    at (8.75,4.75) {ventas \quad num [1:3] 1200 950 1830};
  % 4. Archivos, gráficas, ayuda
  \node[titulo] at (8.75,3.5) {4. Files, Plots, Help\ldots};
  \node[texto, text width=5.2cm] at (8.75,2.95)
    {Explorador de archivos del proyecto, gráficas, paquetes instalados
     (\textit{Packages}), ayuda (\textit{Help}) y visor de dashboards y
     reportes (\textit{Viewer}).};
\end{tikzpicture}
\caption{Los cuatro paneles de RStudio.}
\label{fig:m0-rstudio}
\end{figure}

Los atajos de teclado de la tabla~\ref{tab:m0-atajos} te ahorrarán mucho
tiempo desde el primer día. El más importante es \tecla{Ctrl} +
\tecla{Enter}: ejecuta la línea donde está el cursor (o el texto
seleccionado) y avanza a la siguiente.

\begin{table}[H]
\centering\small
\caption{Atajos de teclado esenciales de RStudio.}
\label{tab:m0-atajos}
\begin{tabularx}{\textwidth}{@{}X l l@{}}
\toprule
\textbf{Acción} & \textbf{Windows y Linux} & \textbf{macOS} \\
\midrule
Ejecutar la línea actual o la selección &
  \tecla{Ctrl} + \tecla{Enter} & \tecla{Cmd} + \tecla{Return} \\
Ejecutar todo el script &
  \tecla{Ctrl} + \tecla{Shift} + \tecla{Enter} &
  \tecla{Cmd} + \tecla{Shift} + \tecla{Return} \\
Insertar el pipe \codigo{\%>\%} &
  \tecla{Ctrl} + \tecla{Shift} + \tecla{M} &
  \tecla{Cmd} + \tecla{Shift} + \tecla{M} \\
Insertar la asignación \codigo{<-} &
  \tecla{Alt} + \tecla{-} & \tecla{Option} + \tecla{-} \\
Comentar o descomentar líneas &
  \tecla{Ctrl} + \tecla{Shift} + \tecla{C} &
  \tecla{Cmd} + \tecla{Shift} + \tecla{C} \\
Nuevo script &
  \tecla{Ctrl} + \tecla{Shift} + \tecla{N} &
  \tecla{Cmd} + \tecla{Shift} + \tecla{N} \\
Guardar el script & \tecla{Ctrl} + \tecla{S} & \tecla{Cmd} + \tecla{S} \\
Autocompletar nombres de funciones y objetos &
  \tecla{Tab} & \tecla{Tab} \\
Ver la ayuda de la función bajo el cursor & \tecla{F1} & \tecla{F1} \\
Limpiar la consola & \tecla{Ctrl} + \tecla{L} & \tecla{Ctrl} + \tecla{L} \\
Reiniciar R (sesión limpia) &
  \tecla{Ctrl} + \tecla{Shift} + \tecla{F10} &
...
\begin{rnoejecutar}[title=Paquetes obligatorios del curso]
paquetes <- c(
...
\begin{rnoejecutar}[title=Paquetes opcionales]
install.packages(c(
...
\begin{rcode}[title=Verificación del entorno del curso]
...
Al ejecutar \codigo{library(tidyverse)} verás además un mensaje de
bienvenida con la lista de paquetes que se cargaron y un par de avisos de
``conflictos''; no es un error y no lo reproducimos en el libro (el
Módulo~\ref{cap:fundamentos} lo explica). Tus números de versión serán
...
La carpeta \archivo{latex/codigo/} contiene un script de R por módulo
(\archivo{01-fundamentos.R}, \archivo{02-manipulacion-datos.R}, etc.) con
todo el código ejecutable de ese módulo, en el mismo orden que en el libro.
...
referencias cruzadas (``Solución en la página\ldots'') queden correctos. Si
ya usas R, otra opción es el paquete \paquete{tinytex}, que instala una
distribución ligera de \LaTeX{} y descarga sola los paquetes que falten (el
Módulo~\ref{cap:reportes} lo usa para los reportes en PDF):
