...
{\sloppy Para traer una tabla \emph{completa} a R usa \codigo{dbReadTable()}:
\codigo{tiendas\_bd <- dbReadTable(con, "tiendas")} devuelve un data frame
normal con las 10 tiendas, listo para \paquete{dplyr} o \paquete{ggplot2}.\par}
...
{\sloppy \codigo{SELECT *} trae todas las columnas; es cómodo para explorar (por
ejemplo, \codigo{SELECT * FROM transacciones LIMIT 5} para ver cinco filas),
pero en reportes conviene listar solo las columnas que necesitas.\par}
...
segunda muestra los primeros tres ordenados por ciudad y nombre.
...
solo 71 tickets). Aunque la tabla ya trae una columna \codigo{mes}, es
buena práctica saber calcularla desde la fecha: muchas tablas reales solo
tienen la fecha.

Para calcular con fechas, \codigo{date(fecha, '+1 year')} suma un intervalo
(también acepta \codigo{'+7 days'}, \codigo{'-1 month'} o
\codigo{'start of month'}) y la resta de dos \codigo{julianday()} da la
diferencia en días; por ejemplo,
\codigo{julianday('2023-12-31') - julianday(fecha\_registro)} es la
antigüedad de cada cliente al cierre del año.
...
{\sloppy Fíjate en los alias de tabla: \codigo{transacciones AS t} permite escribir
...
La subconsulta \codigo{(SELECT AVG(precio\_catalogo) FROM productos)} se
calcula primero (da \$416.92) y la consulta externa compara cada producto
contra ese valor. Una subconsulta puede ir en el \codigo{WHERE}, en el
\codigo{FROM} o incluso en el \codigo{SELECT}, siempre que en este último
caso devuelva un solo valor.
...
{\sloppy dbplyr traduce muchas funciones de R (aritmética, \codigo{if\_else()},
\codigo{case\_when()}, \codigo{substr()}, \codigo{paste0()},
\codigo{min\_rank()}, \codigo{lag()}, \codigo{cumsum()}, las funciones de
agregación\ldots), pero no todas. Por ejemplo, \codigo{month(fecha)} de \paquete{lubridate}
se traduce a \codigo{strftime('\%m', fecha)} en SQLite. Algunas reglas
prácticas:\par}
...
producto con un \codigo{producto\_id} que ya existe, la llave primaria lo
impide con el error \codigo{UNIQUE constraint failed}. Lo comprobarás en la
sección de ETL y en la de errores comunes.
...
\begin{rsalida}
[1] "ventas_2023_s1.csv" "ventas_2023_s2.csv"
\end{rsalida}
...
y ticket promedio (mostramos los tres primeros meses; en el caso práctico
la usaremos completa). La vista ancha \codigo{vw\_ventas\_detalle} es
útil para analistas que no quieren escribir \emph{joins}: consultan una sola
``tabla'' con todo, por ejemplo
\codigo{SELECT zona, SUM(total) FROM vw\_ventas\_detalle GROUP BY zona}.
...
{\sloppy Reinicia R (\menu{Session > Restart R}) para que lea el archivo. A partir de
ahí, \codigo{Sys.getenv("SQLSERVER\_USER")} devuelve
\codigo{"analista\_bi"} y tu script no contiene ningún secreto:\par}
...
consultar, y responder con ella seis preguntas para la junta mensual:
\begin{enumerate}
  \item ¿Cómo evolucionaron las ventas y el margen mes a mes, y cuánto
        crecimos contra el mes anterior?
...
\subsubsection{Pregunta 1. Ventas, margen y crecimiento por mes}
...
\begin{rsalida}
   mes nombre_mes tickets   ventas margen_pct crec_pct
1    1      enero      98 82559.66       41.7       NA
2    2    febrero      79 65899.34       44.3    -20.2
3    3      marzo      91 90930.61       51.2     38.0
4    4      abril      71 57037.74       34.7    -37.3
5    5       mayo      88 93910.21       52.8     64.6
6    6      junio      84 88509.12       47.5     -5.8
7    7      julio      83 86456.05       47.6     -2.3
8    8     agosto      75 78929.49       42.2     -8.7
9    9 septiembre      86 68855.87       42.8    -12.8
10  10    octubre      74 70004.10       45.3      1.7
11  11  noviembre      80 80506.89       45.4     15.0
12  12  diciembre      91 96393.61       52.8     19.7
\end{rsalida}
...
fue malo por partida doble: vendió 37.3\% menos que marzo \emph{y} con menor
margen; mayo rebotó 64.6\% y el año cerró con dos meses de crecimiento
(15.0\% en noviembre y 19.7\% en diciembre). La vista con los KPIs más una
función de ventana resolvieron la pregunta en una sola consulta. ¿Podemos confiar en estas cifras? Una buena práctica es
...
\subsubsection{Pregunta 6. Fin de semana contra entre semana}
...
Con estas consultas tienes el material de un reporte ejecutivo
...
\begin{hazlotu}
Agrega una séptima pregunta: ¿qué método de pago predomina en cada zona?
...
\subsubsection{Tablas, columnas y llaves}
...
\begin{itemize}
  \item \textbf{no such table / no such column}: la tabla o la columna no
        existe, casi siempre por un error de dedo o porque estás conectado
        a otra base. Revisa con \codigo{dbListTables(con)} y
        \codigo{dbListFields(con, "tabla")}. Recuerda que, tras reconstruir
        la base, las tablas crudas (\codigo{transacciones},
        \codigo{clientes}\ldots) ya no existen.
  \item \textbf{La tabla ya existe}: \codigo{dbWriteTable()} se niega a
        pisarla. \codigo{overwrite = TRUE} la reemplaza (¡perdiendo llaves y
        reglas si la creaste con \codigo{CREATE TABLE}!) y
        \codigo{append = TRUE} agrega filas. Para tablas diseñadas por ti,
        usa \codigo{dbAppendTable()}.
  \item \textbf{UNIQUE constraint failed}: intentaste cargar filas cuya
        llave primaria ya existe (típico al correr dos veces un script).
        Usa carga incremental o borra primero lo que vas a recargar.
\end{itemize}
