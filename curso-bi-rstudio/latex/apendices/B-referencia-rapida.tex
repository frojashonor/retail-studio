...
\codigo{as.numeric(} \codigo{as.character(f))} & Convierte un factor numérico a número real. & Sin \codigo{as.character()} obtendrías los códigos internos 1, 2, 3\ldots \\
\end{longtable}
\endgroup
...
Estimar el monto de compra o el precio de una casa & Regresión & Regresión lineal, bosque aleatorio & \codigo{lm()}, \codigo{randomForest()} & RMSE, MAE, R\textsuperscript{2} \\
...
\codigo{valueBoxOutput("x")} & \codigo{renderValueBox(\{...\})} & Tarjeta de KPI (\paquete{shinydashboard}). \\
...
\begin{itemize}
\item \codigo{html\_document}: interactivo; admite \codigo{toc},
  \codigo{toc\_float} y \codigo{code\_folding}.
\item \codigo{pdf\_document}: requiere LaTeX (p. ej., \paquete{tinytex}).
\item \codigo{word\_document}: editable por otras áreas.
\item \codigo{powerpoint\_presentation} e \codigo{ioslides\_presentation}:
  presentaciones.
\item \codigo{flexdashboard::flex\_dashboard}: tablero estático.
\end{itemize}
Un reporte parametrizado se ve así:
...
Dentro del documento usas \codigo{params\$tienda}. Para generar un
reporte por tienda, llama a \codigo{render()} (del paquete
\paquete{rmarkdown}) dentro de un bucle:

\begin{rnoejecutar}[title={Un reporte por tienda}]
for (t in 1:10) {
...
\texttt{Sheet 'Ventas2024' not found} & Nombre de hoja de Excel incorrecto. & Lista las hojas con \codigo{excel\_sheets()}. \\
